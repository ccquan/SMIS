...
?>
	<div class="container" style="padding-top: 40px;"><!--整个盒子居中-->
			<table class="table table-bordered text-center">
				...
				<?php
					include("../connect.php");
					$s_sql ="SELECT * FROM `kc_info`  \n"."ORDER BY `kc_info`.`kc_id` ASC";
					global $con;
					...
					if($count!=0){
						while ($userinfo = mysqli_fetch_array($res)) {
                    		echo "<tr>";
							echo "<td>$userinfo[0]</td><td>$userinfo[1]</td><td>$userinfo[2]</td>";
			  ...
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body">
			       	<form id="xg">
			       		<div class="form-group">
			       			<input type="text" placeholder="课程号" name="id" class="form-control" readonly/>
			       		</div>
			       		<div class="form-group">
			       			<input type="text" placeholder="名称" class="form-control" name="name"/>
			       		</div>
						<div class="form-group">
			       			<input type="text" placeholder="课时" class="form-control" name="sex"/>
			       		</div>
			       	</form>
			      </div>
			  ...
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body">
			       	<form id="tj">
			       		<div class="form-group">
			       			<input type="text" placeholder="课程号" name="id" class="form-control"/>
			       		</div>
			       		<div class="form-group">
			       			<input type="text" placeholder="名称" class="form-control" name="name"/>
			       		</div>
	...
	$(".que_add").click(function(){//点开增加按钮弹出的模态框后的确定按钮事件
				...
				if (arr[0]==""||arr[1]==""||arr[2]==""||arr[3]=="") {//判断内容是否为空，否则弹窗"请输入一点东西"
					...
					$.ajax({
					type:"post",
					url:"action/actionKC.php",
					data:"action=0&id="+arr[0]+"&name="+arr[1]+"&time="+arr[2]+"&submit=1",
					dataType:"text",
		...
		var _this=null;
			...
			$(".que_update").click(function(){//同增加事件
				...
				$.ajax({
					type:"post",
					url:"action/actionKC.php",
					data:"action=2&id="+arr[0]+"&name="+arr[1]+"&time="+arr[2],
					dataType:"text",
					success:function(data){
						if(data.indexOf("true")!=-1){
