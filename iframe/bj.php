...
?>
	<div class="container" style="padding-top: 40px;"><!--整个盒子居中-->
			<table class="table table-bordered text-center">
				...
				<?php
					include("../connect.php");
					$s_sql ="SELECT * FROM `bj_info`  \n"."ORDER BY `bj_info`.`bj_id` ASC";
					global $con;
					...
					if($count!=0){
						while ($userinfo = mysqli_fetch_array($res)) {
                    		$bj_id= $userinfo["bj_id"];
                    		$name= $userinfo["bj_name"];
                    		$teacher= $userinfo["bj_teacher"];
                    		echo "<tr>";
							echo "<td>$bj_id</td><td>$name</td><td>$teacher</td>";
			  ...
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body">
			       	<form id="xg">
			       		<div class="form-group">
			       			<input type="text" placeholder="班级号" class="form-control" readonly/>
			       		</div>
			       		<div class="form-group">
			       			<input type="text" placeholder="名称" class="form-control" />
			       		</div>
						<div class="form-group">
			       			<input type="text" placeholder="班主任" class="form-control"/>
			       		</div>
			       	</form>
			      </div>
			  ...
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      ...
			      <div class="modal-body">
			       	<form id="tj">
			       		<div class="form-group">
			       			<input type="text" placeholder="班级号" class="form-control"/>
			       		</div>
			       		<div class="form-group">
			       			<input type="text" placeholder="名称" class="form-control" />
			       		</div>
	...
	$(".que_add").click(function(){//点开增加按钮弹出的模态框后的确定按钮事件
				...
				if (arr[0]==""||arr[1]==""||arr[2]=="") {//判断内容是否为空，否则弹窗"请输入一点东西"
					alert("请输入一点东西");
				} else{//是则加入到table中（表格）并把模态框关闭
					$.ajax({
					type:"post",
					url:"action/actionBJ.php",
					data:"action=0&id="+arr[0]+"&name="+arr[1]+"&teacher="+arr[2]+"&submit=1",
					dataType:"text",
					success:function(data){
						if(data.indexOf("true")!=-1){
							str='<tr><td>'+arr[0]+'</td><td>'+arr[1]+'</td><td>'+arr[2]+'</td><td><button class="btn btn-primary btn-xs update" data-toggle = "modal" data-target = "#updateModal">修改</button><button class="btn btn-danger btn-xs del">删除</button></td></tr>'
		...
		var _this=null;
			...
			$(".que_update").click(function(){//同增加事件
				...
				$.ajax({
					type:"post",
					url:"action/actionBJ.php",
					data:"action=2&id="+arr[0]+"&name="+arr[1]+"&teacher="+arr[2],
					dataType:"text",
					success:function(data){
						if(data.indexOf("true")!=-1){
