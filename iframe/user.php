...
?>
				...
				<?php
					...
					if($count!=0){
						while ($userinfo = mysqli_fetch_array($res)) {
                    		echo "<tr>";
							echo "<td>$userinfo[1]</td><td>$userinfo[2]</td>";
	...
	$(".que_add").click(function(){//点开增加按钮弹出的模态框后的确定按钮事件
				...
				if (arr[0]==""||arr[1]=="") {//判断内容是否为空，否则弹窗"请输入一点东西"
					alert("请输入一点东西");
				} else{//是则加入到table中（表格）并把模态框关闭
					$.ajax({
					type:"post",
					url:"action/actionUSER.php",
					data:"action=0&username="+arr[0]+"&password="+arr[1]+"&submit=1",
					dataType:"text",
					success:function(data){
						if(data.indexOf("true")!=-1){
							str='<tr><td>'+arr[0]+'</td><td>'+arr[1]+'</td><td><button class="btn btn-primary btn-xs update" data-toggle = "modal" data-target = "#updateModal">修改</button><button class="btn btn-danger btn-xs del">删除</button></td></tr>'
		...
		$(document).on("click",".del",function(){//找到点击的类目为del的按钮实现删除
			...
			if(a.length>1){
				...
				$.ajax({
						type:"post",
						url:"action/actionUSER.php",
						data:"action=1&username="+a.innerText,
						dataType:"text",
						success:function(data){
							if(data.indexOf("true")!=-1){
		...
		var _this=null;
			...
			$(".que_update").click(function(){//同增加事件
				...
				$.ajax({
					type:"post",
					url:"action/actionUSER.php",
					data:"action=2&username="+arr[0]+"&password="+arr[1],
					dataType:"text",
					success:function(data){
						if(data.indexOf("true")!=-1){
